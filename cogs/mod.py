...
class mod():
    ...
    async def on_member_join(self, user):
        ...
        if not send_channel:
            return
        await send_channel.send(x['message'].replace('$name$', user.name).replace('$mention$', user.mention).replace('$server$', user.guild.name).replace('$members$', len(guild.Members))
